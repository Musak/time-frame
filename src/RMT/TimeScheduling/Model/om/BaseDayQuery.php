 ...
 * @method DayQuery orderByValue($order = Criteria::ASC) Order by the value column
 *
 * @method DayQuery groupById() Group by the id column
 * @method DayQuery groupByValue() Group by the value column
 *
 ...
 * @method array findByValue(string $value) Return Day objects filtered by the value column
 */
abstract class BaseDayQuery extends ModelCriteria
{
     ...
     */
    public function __construct($dbName = 'default', $modelName = 'RMT\\TimeScheduling\\Model\\Day', $modelAlias = null)
    {
    ...
    {
        if ($dayInterval instanceof DayInterval) {
            return $this
                ->addUsingAlias(DayPeer::ID, $dayInterval->getDayId(), $comparison);
        } elseif ($dayInterval instanceof PropelObjectCollection) {
            return $this
                ->useDayIntervalQuery()
                ->filterByPrimaryKeys($dayInterval->getPrimaryKeys())
                ->endUse();
        } else {
            throw new PropelException('filterByDayInterval() only accepts arguments of type DayInterval or PropelCollection');
        }
    }

    /**
     * Adds a JOIN clause to the query using the DayInterval relation
     *
     * @param     string $relationAlias optional alias for the relation
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return DayQuery The current query, for fluid interface
     */
    public function joinDayInterval($relationAlias = null, $joinType = Criteria::LEFT_JOIN)
    {
        $tableMap = $this->getTableMap();
        $relationMap = $tableMap->getRelation('DayInterval');
        ...
        if ($relationAlias) {
            ...
            $this->addJoinObject($join, 'DayInterval');
        }
    ...
    /**
     * Use the DayInterval relation DayInterval object
     *
     * @see       useQuery()
     *
     * @param     string $relationAlias optional alias for the relation,
     ...
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return   \RMT\TimeScheduling\Model\DayIntervalQuery A secondary query class using the current class as primary query
     */
    public function useDayIntervalQuery($relationAlias = null, $joinType = Criteria::LEFT_JOIN)
    {
        return $this
            ->joinDayInterval($relationAlias, $joinType)
            ->useQuery($relationAlias ? $relationAlias : 'DayInterval', '\RMT\TimeScheduling\Model\DayIntervalQuery');
    }
